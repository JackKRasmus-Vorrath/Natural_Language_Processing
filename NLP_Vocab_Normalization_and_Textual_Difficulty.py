...
def text_scores(text):
    ...
    vocab_size = len(text_lower_alpha)
    ...
    vocab_size_long = len(text_lower_alpha_long)
    
    
    #diversity score: distinct types / n_tokens
    div_score = len(set(w.lower() for w in text if w.isalpha()))/vocab_size
...
if __name__ == '__main__':
	...
	#return all html 'a' tags whose 'href' attributes match the regular expression
	links = soup.findAll('a', attrs={'href': re.compile("^//www.")})
	
	#create tuple list of the e-book hyperlinks to UTF-8 text files and cleaned strings of their corresponding titles
	urls = []
	for link in links:
		link = ('http:' + str(link['href']) + '.txt.utf-8', link.get_text().replace('\n',' '))
		urls.append(link)
	...
	for url in urls:
		...
		try:
			...
			continue
		#tokenize the text
		word_tokens = word_tokenize(decoded_text)
